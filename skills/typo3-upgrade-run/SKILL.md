...
description: >-
  End-to-end TYPO3 upgrade run: takes a v12 or v13 site to 14.3 LTS inside a local DDEV
  clone under two contracts. Invariance: the migration must be provably invisible to
  visitors, measured against a baseline frozen before any change. Elevation: separately
  approved performance, SEO, accessibility and security work may start only after
  invariance closes. Owns the version constraints, the PHP target and the process; consults
  typo3-v14-reference for API detail. Every step runs as one loop protocol with
  preconditions, a snapshot rollback anchor, bounded iterations, classified findings and
  abort conditions, documented per loop in its own numbered directory. Covers sitemaps,
  Bootstrap 5, extensions, PHP 8.4+, Rector, Fractor, PHPStan, wizards, workspaces, Vite,
  Solr, Visual Editor, CKEditor RTE, security headers, Lighthouse and a Word KPI report.
