...
| **CSS written against whatever markup happened to exist** | These stylesheets predate the templates being tidied, so selectors count elements that were never meant to be structural — a redundant wrapper, an accidental nesting level. Removing the wrapper is correct and moves the layout. Before any markup cleanup, grep the CSS for the element you are removing a level of, and measure bounding boxes before/after. See `known-problems.md`. |
| **Images already at their final dimensions in `fileadmin`** | Editors upload pre-scaled files, so TYPO3 sees no resize to perform and never re-encodes them — the configured quality never applies and a 250 KB upload stays a 250 KB download. Usually the single biggest performance win available. |
...
| **TypoScript that exists only in the database** | These sites were built through the backend, so `sys_template.config`/`constants` carry real configuration that is on no disk anywhere. Every file-based migration tool — Fractor included — is blind to it and still reports success. Sweep the DB at P00; a green tool run is not coverage. |
